...
void Nitrade::HistoricSimulator::optimise(int cpus, std::vector<std::string> assets, bool runningPL)
{
	...
	remove(DEFAULT_TRADES_BIN);
	remove(DEFAULT_TRADES_DATA_BIN);
	remove(DEFAULT_PL_BIN);
	...
	{		
		...
		tm->writeTradeDataToBinary(DEFAULT_TRADES_DATA_BIN, startTradeId);
		startTradeId = tm->writeTradesToBinary(DEFAULT_TRADES_BIN, startTradeId);
		
		if (runningPL)
			tm->writeRunningPLToBinary(DEFAULT_PL_BIN);
...
void Nitrade::HistoricSimulator::optimise(std::string assetName, bool runningPL)
{
	...
	//write all trades to a binary file
	tradeManager->writeTradesToBinary(DEFAULT_TRADES_BIN);
	tradeManager->writeTradeDataToBinary(DEFAULT_TRADES_DATA_BIN);
	if (runningPL)
		tradeManager->writeRunningPLToBinary(DEFAULT_PL_BIN);
