...
public:
	...
	MOCK_METHOD1(getAssetData, unique_ptr<IAssetData>(Strategy* strategy));
	MOCK_METHOD1(getBinaryChunkReader, unique_ptr<IBinaryChunkReader>(const string dataPath));
	MOCK_METHOD2(getStrategySet, unique_ptr<IStrategySet>(Strategy* strategyDefinition, IAsset* asset));
